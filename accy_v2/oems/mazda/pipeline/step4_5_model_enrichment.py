...
def run(
    ...
    # Extract model keywords from meta_data["model_name"] (extracted in Step 1 from cell (0,0))
    model_keywords = _extract_model_keywords(model_name, pipeline_logger)

    # Extract fuel_type from sheet_name if applicable
    fuel_type = None
    try:
    ...
    pipeline_logger.debug(
        ...
        f"model_keywords={model_keywords}, fuel_type={fuel_type}"
    )

    # Batch lookup: one per unique trim (combines model_keywords + fuel_type + trim_keywords)
...
def _batch_lookup_model_numbers(
    ...
    for trim in trims:
        ...
        try:
            # Path: accy_v2/oems/mazda/pipeline/step4_5_model_enrichment.py
...
def _extract_model_keywords(model_name: str, pipeline_logger: PipelineLogger) -> List[str]:
    """
    Extract model keywords for Mazda from model_name.

    For Mazda, model_name comes from meta_data (e.g., "CX-90", "CX-90_ES").
    Remove year keywords (4-digit numbers) as they're used for filtering only.

    Args:
        model_name: Model name from meta_data
        pipeline_logger: Logger

    Returns:
        List of model keywords
    """
    model_name_str = model_name.lower().strip() if model_name else ""

    if model_name_str:
        # Split by underscore or dash if present
        parts = [kw.strip() for kw in model_name_str.replace("-", "_").split("_") if kw.strip()]
        # Filter out year keywords (4-digit numbers)
        keywords = [kw for kw in parts if not (len(kw) == 4 and kw.isdigit())]
        pipeline_logger.debug(f"Extracted model keywords from model_name: {keywords}")
        return keywords

    pipeline_logger.warning("No model_name found for Mazda")
    return []
