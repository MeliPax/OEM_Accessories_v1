...
def _merge_compound_model_names(tokens: List[str], oem_config: Dict[str, Any], pipeline_logger=None) -> List[str]:
    ...
    Args:
        ...
        oem_config: OEM configuration dict containing classifier_config path
    ...
    try:
        # Get the resolved classifier_config path from OEM config (resolved by ModularConfigLoader)
        classification_path = oem_config.get("classifier_config")

        if not classification_path:
            if pipeline_logger:
                pipeline_logger.debug(f"[MERGE] No classifier_config found in oem_config")
            return tokens
